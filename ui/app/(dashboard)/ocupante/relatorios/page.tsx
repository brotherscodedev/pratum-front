...
const RelatoriosPage = () => {
  ...
  const { loading, call: listNotificacoes } = useAsync(async (pagination: PaginationState, filtros: FiltrosRelatoriosType) => {
    ...
    try {
      ...
      toast({ description: "Não foi possível carregar os relatórios", variant: "erro", position: "top" })
    }
  }, []);
  ...
  const marcarNotificacaoLida = useCallback( async (notificacaoId: number) => {
    notificacaoService.markRead(notificacaoId)
      .then(() => toast({description: "Relatório lido.", variant: "sucesso"}))
      .catch(() => toast({description: "Não foi possível marcar o relatório como lido.", variant: "erro", position: "top"}))
  }, [])
