...
const ResponderNotificacaoModal = ({
  ...
  const [fileUrlMensagem, setFileUrlMensagem] = useState<String>("");
  ...
  const handleSave = async () => {
    ...
    try {
        notificacaoService.respondNotification({
          ...
          mensagem: `${resposta} ${fileUrlMensagem}`  ,
          arquivos: [fileSend]
        })
            ...
            .catch((error) => {
              ...
              toast({ description: errorMessage, variant: "erro", position: "top" })})
      setResposta("");
      onClose();
    } catch (error) {
      toast({
  ...
  return (
    ...
    >
      <div className="space-y-4 bg-greenTertiary">
        {exibirMensagemAguarde ? <span className="block pl-4 mt-6 text-white dark:text-white">Aguarde a resposta da concessionária</span> : null}
        ...
        <div className="flex justify-end mt-8 pb-8 mr-8 gap-2">
          ...
          {habilitaResponder && !exibirMensagemAguarde ? <Button variant="secondary" onClick={handleSave} disabled={isSubmitting}>
